...
type OrderItem = { id: string; size: string; quantity: number; price: number; thumbnail: string; name: string };
    ...
    return (
      <main className="section">
        <div className="container text-center">
          ...
          <Link href="/" className="btn inline-block mt-4 px-5 py-2">Continue Shopping</Link>
        </div>
      </main>
    );
  }
      ...
      <div className="container text-center">
        <div className="inline-block text-left card p-8">
          <div className="text-center">
            ...
            <h1 className="heading-serif mt-2 text-3xl md:text-4xl font-semibold">Thank you. You&apos;re all set.</h1>
            ...
            <ul className="space-y-2">
              {order.items.map((it) => (
                <li key={`${it.id}-${it.size}`} className="flex items-center gap-3">
                  <img src={it.thumbnail} alt={it.name} className="w-12 h-12 rounded-md object-cover" />
          ...
          <div className="mt-6 text-center">
            <Link href="/" className="btn inline-block px-6 py-3">Continue Shopping</Link>
          </div>
        </div>
      </div>
    </main>
